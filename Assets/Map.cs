...
public class Map : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (step == 1) {
            if (!this.moved) {
                this.touyamaomoko.GetComponent<RectTransform>().localPosition = new Vector3(-126, -285, 0);
                this.moved = true;
            }
        }
        if (step == 2) {
            if (!this.moved) {
                this.touyamaomoko.GetComponent<RectTransform>().localPosition = new Vector3(-88, 179, 0);
                this.moved = true;
            }
        }
        if (step == 3) {
        ...
        if (step == 2) {
            if (this.touyamaomoko.GetComponent<RectTransform>().localPosition.y >= -250) {
                this.moveTo3();
            } else {
                SceneManager.LoadScene("buttle");
